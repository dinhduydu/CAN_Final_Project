...
CAN_HandleTypeDef hcan1; /* this variable would be extern into 1 file common in DRIVER layer, 
                          ex: extern 1 variable CAN1 handler, definit it into can_common.c */
...
void CAN_Filter_Config(void)
{
  ...
  can1_filter_init.FilterMaskIdHigh = 0X01C0; /* 0X651 -> 1C */
 ...
 void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if(RxHeader.StdId == 0x65D && RxHeader.RTR == 0 )
  {
    ...
    LED_Manage_Output(rcvd_msg[0]); /* Send only one byte -> index 0 */
  ...
  else if ( RxHeader.StdId == 0x651 && RxHeader.RTR == 1)
  {
    ...
    //return; /* after sending response it has to return but would do not this */
  }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if ( req_counter  == 4)
  {
    /* N1 sending Remote frame to N2 every 4 second. */
    TxHeader.DLC = 2; /* N1 demanding 2 bytes of reply */
    TxHeader.StdId = 0x651;
    TxHeader.IDE   = CAN_ID_STD;
    TxHeader.RTR = CAN_RTR_REMOTE;

    if( HAL_CAN_AddTxMessage(&hcan1,&TxHeader,&message,&TxMailbox) != HAL_OK) /* message no meaning here because Remote Frame */
    {
      Error_handler();
    }
    req_counter = 0;
  ...
  {
    CAN1_Tx();
    req_counter++; /* incrementing Whenever I send  the Data Frame. */
  }
}
...
void Send_response(uint32_t StdId)
{
  ...
  uint8_t response[2] = { 0xAB,0XCD}; /* it can be temperature value, engine state, sensor value */
